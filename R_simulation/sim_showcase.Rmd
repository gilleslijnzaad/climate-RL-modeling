...
The learning rate is determined by the difference between rating and belief, multiplied by some weight. We define:
\begin{alignat*}{2}
{LR} &= w_{LR} \cdot {LR}'                   &\quad &\text{with } {LR}  \in [0, w_{LR}] \\
{LR}' &= \min(\text{diff}/9 + 1, 1) &\quad &\text{with } {LR}' \in [0, 1] \\
\text{diff} &= R - \text{belief}             &\quad &\text{with diff} \in [-9, 9]
\end{alignat*}

See figure below. In words: the learning rate is zero when the rating is maximally worse than the belief ($1 - 10 = -9$), and one when the rating is greater than or equal to the belief. In between, the deviation from belief is positively correlated to the learning rate.

Outstanding question: should the *theoretical* "worst case scenario" be used here? A rating of 1 and a belief of 10 is an unlikely scenario. 
