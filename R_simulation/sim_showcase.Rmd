...
{LR}' &= \min(\frac{1}{9} \cdot \text{diff} + 1, 1) &\quad &\text{with } {LR}' \in [0, 1] \\
\text{diff} &= R - \text{belief}             &\quad &\text{with diff} \in [-9, 9]
\end{alignat*}

The relationship between LR' and the deviation from belief is also illustrated in the figure below.
...
TODO: illustrate how LR changes over trials based on ..., to verify if it works as intended
