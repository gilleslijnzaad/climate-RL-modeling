...
model_dat_JSON <- function(params, model_dat) {
  ...
  T <- n_trials
  N <- n_participants
  list_param_settings <- list(LR, inv_temp, initQF, initQU, mu_R, sigma_R, T, N)
  names(list_param_settings) <- c("LR", "inv_temp", "initQF", "initQU", "mu_R", "sigma_R", "T", "N")

  participants <- rep(1:N, each = T)
  choice <- as.numeric(model_dat$choice == "U") + 1
  R <- model_dat$R
  list_dat <- list(participants, choice, R)
  names(list_dat) <- c("j", "choice", "R")
  return(c(toJSON(list_param_settings), toJSON(list_dat)))
}
```
...
dat_std <- run_model()
write(model_dat_JSON(params_std, dat_std)[1], "sim_param_settings.json")
write(model_dat_JSON(params_std, dat_std)[2], "sim_dat.json")
