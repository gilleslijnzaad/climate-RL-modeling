...
I define rating as a function in order to easily edit it later on. For now, I say that rating is drawn from a truncated normal distribution (bounds [1, 10]) with mean 5 and standard deviation 3.
```{r fun-rating}
library(truncnorm)
...
## 2b. Data to JSON for Stan
I want to plug simulated data into Stan for model recovery. To this end, I need to convert the relevant data points to JSON format. NOTE: this does not include participant number yet, because the most simple Stan model does not either.
```{r dat-to-JSON}
library(rjson)
model_dat_JSON <- function(params, model_dat) {
  ...
  R <- model_dat$R
  my_list <- list(LR, inv_temp, initQF, initQU, T, choice, R)
  names(my_list) <- c("LR", "inv_temp", "initQF", "initQU", "T", "choice", "R")
  return(toJSON(my_list))
}
```

# 3. Run & inspect
## 3.0 Standard settings
...
## 3.1 Differential initial values
...
## 3.3 Varying inverse temperature
...
# 4. Modifying the model: confirmation bias
...
### 4.2.2 Draw rating from separate distributions
