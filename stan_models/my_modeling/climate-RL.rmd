...
# Some next steps
1. Create a `generated quantities` block to see what Q values, choices and ratings the model predicts. Maybe this will help explain the "bug";
1. Loop over participants, where each participant has the same parameter values (so only one set of parameter values gets estimated). This is just to make sure that I properly implement the participant loop. Maybe this changes something about the "bug";
