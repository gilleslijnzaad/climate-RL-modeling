...
## Run simulation
```{r param-settings}
param_settings <- list(LR = 0.25, inv_temp = 0.2)
```

Parameter settings are:

- LR = `r param_settings[["LR"]]`
- inv_temp = `r param_settings[["inv_temp"]]`
...
Full model code here: [sandbox_model.stan](https://github.com/gilleslijnzaad/climate-RL-modeling/blob/main/stan_models/sandbox/sandbox_model.stan).
```{r fit-model}
...
if (refit) {
  ...
  fit$save_object(file = "sandbox_model_fit.rds")
} else {
  fit <- readRDS(file = "sandbox_model_fit.rds")
}
```
...
```{r plot-fit}
source("../../plot_utils.R")
posterior_density_plot(fit, c("LR", "inv_temp"), param_settings)
```

```{r pred, eval=FALSE, include=FALSE}
